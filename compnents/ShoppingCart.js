...
export default function ShoppingCart() {
  ...
  async function handleClick(event) {
    ...
    if (cartCount > 0) {
      setStatus("loading");
      try {
        const response = await axios.post("/api/checkout_sessions", items);
        const { data } = response;
        console.log(data);
  ...
  return (
    ...
    >
      {cartCount && cartCount > 0 ? (
        <>
          <div>Total: {totalPrice ? totalPrice.toFixed(2) : "0.00"}</div>
          ...
          <article className="mt-3 flex flex-col">
            ...
            <button
              onClick={handleClick}
              className="bg-emerald-50 hover:bg-emerald-500  transition-colors duration-500 text-emerald-500 py-3 px-5 rounded-md w-100 disabled:bg-slate-300 disabled:cursor-not-allowed disabled:text-white"
              disabled={
                (totalPrice && totalPrice < 30) ||
                (cartCount && cartCount > 20) ||
                status === "no-items"
